...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    const transaction = await buildTransaction(new PublicKey(account), new PublicKey(reference))
    return NextResponse.json(
      {
    ...
    return NextResponse.json({ error: 'Error Building Transaction' }, { status: 500 })
  }
}
...
async function buildTransaction(account: PublicKey, reference: PublicKey) {
  ...
  const associatedTokenAccount = getAssociatedTokenAddressSync(mintPDA, account, false, TOKEN_2022_PROGRAM_ID)
