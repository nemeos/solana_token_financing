...
export default function SolanaPay() {
  ...
  useEffect(() => {
    ...
    const apiUrl = `${location.protocol}//${location.host}/api/increment?reference=${reference.toBase58()}`
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        ...
        // Toast notification
        toast.success(
          <a href={`https://solana.fm/tx/${signatureInfo.signature}?cluster=devnet-alpha`} target="_blank">
            View on SolanaFM
          </a>,
          {
            style: {
